"""
Coinbase listing processor - reads raw scrape data and formats it into clean output
...
class CoinbaseProcessor:
    """Process raw Coinbase scrape data into clean listing format"""
    ...
    def determine_venue(self, listing_type):
        """Determine specific venue based on listing type"""
        return f"{self.exchange_name} Spot"
    
    def process_file(self, input_file):
        """Process raw Coinbase scrape JSON file"""
...
def main():
    """Main function to process Coinbase scrape results"""
    import sys
    
    processor = CoinbaseProcessor()
    
    # Default input file
    input_file = 'coinbase_listings.json'
    ...
    if listings:
        ...
        processor.save_to_json(listings, f'coinbase_processed_{timestamp}.json')
        processor.save_to_csv(listings, f'coinbase_processed_{timestamp}.csv')
