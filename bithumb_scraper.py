"""
Bithumb exchange scraper - scrapes listing data from ListedOn.org
"""
...
class BithumbScraper:
    """Scraper for Bithumb exchange listings via ListedOn.org"""
    
    def __init__(self, headless=True):
        self.headless = headless
        self.driver = None
        self.exchange_name = "Bithumb"
        self.url = "https://listedon.org/en/exchange/bithumb"
    ...
    def scrape(self):
        """Scrape Bithumb listings from ListedOn.org - ONLY PAGE 1"""
        all_listings = []
        ...
        try:
            ...
            print(f"[{self.exchange_name}] Scraping: {page_url}")
            ...
            print(f"[{self.exchange_name}] Found {len(rows)} rows")
...
def main():
    """Test the Bithumb scraper"""
    print("="*80)
    print("BITHUMB SCRAPER TEST (via ListedOn.org)")
    print("="*80 + "\n")
    
    scraper = BithumbScraper(headless=False)  # Set to True to hide browser
    listings = scraper.scrape()  # Only scrapes page 1
